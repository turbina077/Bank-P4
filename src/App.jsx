...
const P4_LANDING_ANIM = `
@keyframes p4float {
  0%, 100% { transform: translateY(0) rotateX(9deg) rotateZ(-2deg); }
  50%      { transform: translateY(-11px) rotateX(6deg) rotateZ(-1deg); }
}
@media (prefers-reduced-motion: reduce) {
  [style*="p4float"] { animation: none !important; transform: rotateX(8deg) rotateZ(-2deg) !important; }
}
`;
...
export default function App() {
  ...
  return (
    <>
      <style>{P4_ANIM + P4_LANDING_ANIM}</style>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden text-white" style={{fontFamily: FONT_BODY, background:'radial-gradient(circle at 84% 6%, rgba(103,58,183,.20), transparent 30%), #060609'}}>
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'radial-gradient(ellipse 70% 40% at 50% 30%, rgba(166,107,255,.13), transparent 70%)'
      }} />

      <div className="max-w-md mx-auto min-h-screen px-6 pt-7 pb-6 flex flex-col relative z-10">
        <BrandMark />

        <div className="flex justify-center mt-8 mb-2" style={{perspective:'1400px'}}>
          <div style={{
            width: '78%', maxWidth: 290,
            animation: 'p4float 6s ease-in-out infinite',
            transformStyle: 'preserve-3d'
          }}>
            <div style={{
              position:'relative', width:'100%', aspectRatio:'1.586 / 1', borderRadius:13,
              background:'linear-gradient(150deg, #2A1B3D 0%, #17101F 46%, #0B0710 100%)',
              boxShadow:'0 34px 62px rgba(0,0,0,.72), 0 6px 16px rgba(0,0,0,.5), inset 0 1px 0 rgba(217,190,136,.22), inset 0 -1px 0 rgba(0,0,0,.6), inset 1px 0 0 rgba(255,255,255,.05)',
              overflow:'hidden', padding:'5.5%', display:'flex', flexDirection:'column', justifyContent:'space-between'
            }}>
              <div style={{position:'absolute', top:'-55%', left:'-28%', width:'82%', height:'210%',
                background:'linear-gradient(102deg, transparent 32%, rgba(255,244,222,.10) 50%, transparent 66%)',
                transform:'rotate(14deg)'}} />
              <div style={{position:'relative', display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                <div>
                  <div style={{fontSize:'clamp(9px,3.2vw,12px)', color:'#EDE4F2', fontWeight:500}}>P4 Reserve</div>
                  <div style={{fontSize:'clamp(5px,1.8vw,7px)', letterSpacing:'.26em', color:'#D9BE88', marginTop:2}}>MEMBER</div>
                </div>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" style={{opacity:.75}} aria-hidden="true">
                  <path d="M6 8.5a7 7 0 0 1 0 7" stroke="#E8DCC4" strokeWidth="1.7" strokeLinecap="round"/>
                  <path d="M10 6a10.5 10.5 0 0 1 0 12" stroke="#E8DCC4" strokeWidth="1.7" strokeLinecap="round"/>
                  <path d="M14 3.5a14 14 0 0 1 0 17" stroke="#E8DCC4" strokeWidth="1.7" strokeLinecap="round"/>
                </svg>
              </div>
              <div style={{position:'relative', width:'22%', aspectRatio:'1.35 / 1', borderRadius:5,
                background:'linear-gradient(140deg,#F0DBA4 0%,#D2B573 32%,#A98B4E 68%,#8A6E38 100%)',
                boxShadow:'inset 0 1px 0 rgba(255,250,232,.8), 0 1px 3px rgba(0,0,0,.55)'}}>
                <div style={{position:'absolute',left:0,right:0,top:'30%',height:1,background:'rgba(88,66,26,.75)'}}/>
                <div style={{position:'absolute',left:0,right:0,top:'68%',height:1,background:'rgba(88,66,26,.75)'}}/>
                <div style={{position:'absolute',top:0,bottom:0,left:'30%',width:1,background:'rgba(88,66,26,.75)'}}/>
              </div>
              <div style={{position:'relative'}}>
                <div style={{fontFamily:FONT_MONO, fontSize:'clamp(9px,3.3vw,13px)', letterSpacing:'.13em', color:'#F1E9F5',
                  textShadow:'0 1px 0 rgba(255,255,255,.2), 0 2px 3px rgba(0,0,0,.7)'}}>•••• •••• •••• 6230</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center">
          <button onClick={() => setView('student-login')} className="w-full rounded-[20px] p-[18px] mb-3 flex items-center gap-3.5 border transition-all hover:-translate-y-0.5" style={{background:'linear-gradient(145deg, rgba(132,77,255,.18), rgba(255,255,255,.03))', borderColor:'rgba(157,109,255,.34)', boxShadow:'0 10px 30px rgba(102,58,183,.18)'}}>
            <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{background:'#A66BFF', color:'#100A16'}}><WalletCards size={19}/></div>
        ...
        <div className="pt-7 mt-2 text-center" style={{borderTop:'1px solid rgba(255,255,255,.06)'}}>
          <div className="text-[11px] text-white/28">Prof. Tomás Urbina</div>
        </div>
      </div>
    </div>
  );
...
const compressSquare = (file) => new Promise((resolve, reject) => {
  const reader = new FileReader();
  reader.onload = (e) => {
    const img = new Image();
    img.onload = () => {
...
function Receipt({ data, theme, onClose, toast }) {
  const ref = useRef();
  ...
  const saveImage = async () => {
    try {
      ...
      const line = (txt, x, y, size, color, align, weight) => {
        ctx.fillStyle = color; ctx.textAlign = align || 'left';
        ctx.font = (weight || '400') + ' ' + size + "px 'Manrope', Helvetica, Arial, sans-serif";
        ctx.fillText(txt, x, y);
      };
      line('P4 RESERVE', W / 2, 140, 22, '#D9BE88', 'center', '600');
      line('TRANSFER RECEIPT', W / 2, 175, 13, 'rgba(255,255,255,.42)', 'center');
      ctx.strokeStyle = 'rgba(255,255,255,.12)'; ctx.beginPath(); ctx.moveTo(96, 210); ctx.lineTo(W - 96, 210); ctx.stroke();

      line(data.amount.toLocaleString('en-US'), W / 2, 330, 84, '#FFFFFF', 'center', '700');
      line('IGOs', W / 2, 372, 20, '#D9BE88', 'center');

      let y = 470;
      const row = (k, v) => {
        line(k.toUpperCase(), 110, y, 12, 'rgba(255,255,255,.38)');
        line(v, W - 110, y, 17, '#FFFFFF', 'right', '500');
        y += 62;
      };
      row('From', data.from);
      row('To', data.to);
      row('Date', data.date.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' }));
      row('Time', data.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }));
      row('Reference', data.ref);
      if (data.note) row('Note', data.note.slice(0, 26));

      ctx.strokeStyle = 'rgba(255,255,255,.12)'; ctx.beginPath(); ctx.moveTo(96, H - 150); ctx.lineTo(W - 96, H - 150); ctx.stroke();
      line('Classroom simulation · Not a financial instrument', W / 2, H - 105, 12, 'rgba(255,255,255,.32)', 'center');

      const blob = await new Promise(res => canvas.toBlob(res, 'image/png'));
      const file = new File([blob], 'p4-receipt.png', { type: 'image/png' });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: 'P4 Reserve receipt' });
      } else {
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'p4-receipt-' + data.ref + '.png';
        a.click();
        URL.revokeObjectURL(a.href);
        toast && toast('Receipt saved');
      }
    } catch (err) {
      console.error(err);
      toast && toast('Could not create the image', 'bad');
    }
  };

  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 130, background: 'rgba(0,0,0,.82)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 360 }}>
        <div ref={ref} style={{ background: '#0B0710', border: '1px solid rgba(255,255,255,.10)', borderRadius: 22, padding: 26 }}>
          <div style={{ textAlign: 'center', paddingBottom: 18, borderBottom: '1px solid rgba(255,255,255,.10)' }}>
            <div style={{ fontSize: 14, letterSpacing: '.24em', color: '#D9BE88', fontWeight: 600 }}>P4 RESERVE</div>
            <div style={{ fontSize: 10, letterSpacing: '.2em', color: 'rgba(255,255,255,.4)', marginTop: 5 }}>TRANSFER RECEIPT</div>
          </div>
          <div style={{ textAlign: 'center', padding: '26px 0' }}>
            <div style={{ fontSize: 44, fontWeight: 700, color: '#fff', letterSpacing: '-.02em', fontFamily: FONT_MONO }}>{data.amount.toLocaleString('en-US')}</div>
            <div style={{ fontSize: 13, color: '#D9BE88', marginTop: 2 }}>IGOs</div>
          </div>
          {[['From', data.from], ['To', data.to],
            ['Date', data.date.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })],
            ['Time', data.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })],
            ['Reference', data.ref]].concat(data.note ? [['Note', data.note]] : []).map(([k, v]) => (
            <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '9px 0' }}>
              <span style={{ fontSize: 10, letterSpacing: '.16em', color: 'rgba(255,255,255,.38)', textTransform: 'uppercase' }}>{k}</span>
              <span style={{ fontSize: 13, color: '#fff', textAlign: 'right', wordBreak: 'break-word' }}>{v}</span>
            </div>
          ))}
          <div style={{ marginTop: 18, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,.10)', textAlign: 'center', fontSize: 10, color: 'rgba(255,255,255,.3)' }}>
            Classroom simulation · Not a financial instrument
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
          <button onClick={saveImage} style={{ borderRadius: 14, padding: '13px 0', fontSize: 13, fontWeight: 600, background: theme.primary, color: '#09070B', border: 'none' }}>Share</button>
          <button onClick={onClose} style={{ borderRadius: 14, padding: '13px 0', fontSize: 13, background: 'transparent', color: theme.muted, border: '1px solid ' + theme.border }}>Close</button>
        </div>
      </div>
    </div>
  );
...
function CardBack({ student, design, light }) {
  const ink = light ? '#1e1b4b' : '#ffffff';
  return (
    <div className="absolute inset-0 rounded-2xl overflow-hidden" style={{
      background: design.gradient,
      boxShadow: 'inset 0 1px 0 rgba(255,255,255,.18), inset 0 -1px 0 rgba(0,0,0,.5)',
      backfaceVisibility: 'hidden', transform: 'rotateY(180deg)'
    }}>
      <div style={{ position: 'absolute', inset: 0, background: light ? 'rgba(255,255,255,.05)' : 'rgba(0,0,0,.22)' }} />
      <div style={{ position: 'absolute', top: '13%', left: 0, right: 0, height: '19%', background: 'linear-gradient(180deg,#1a1a1a,#000 45%,#1a1a1a)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.08)' }} />
      <div style={{ position: 'absolute', top: '42%', left: '6%', right: '6%' }}>
        <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
          <div style={{
            flex: 1, height: 30, borderRadius: 3,
            background: 'repeating-linear-gradient(135deg,#efe9e2 0 6px,#e2dbd2 6px 12px)',
            boxShadow: 'inset 0 1px 2px rgba(0,0,0,.28)'
          }} />
          <div style={{
            width: 62, height: 30, borderRadius: 3, background: '#f6f2ec',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontFamily: FONT_MONO, fontSize: 13, color: '#1a1a1a', letterSpacing: '.1em',
            boxShadow: 'inset 0 1px 2px rgba(0,0,0,.28)'
          }}>{student.cvv}</div>
        </div>
        <div style={{ fontSize: 6, letterSpacing: '.2em', color: ink, opacity: .6, marginTop: 5, textAlign: 'right' }}>CVV</div>
      </div>
      <div style={{ position: 'absolute', bottom: '7%', left: '6%', right: '6%', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div style={{ fontSize: 6, letterSpacing: '.16em', color: ink, opacity: .55, maxWidth: '68%', lineHeight: 1.5 }}>
          NOT A FINANCIAL INSTRUMENT · CLASSROOM SIMULATION · P4 RESERVE
        </div>
        <div style={{ fontSize: 9, letterSpacing: '.1em', color: ink, opacity: .8 }}>P4</div>
      </div>
    </div>
  );
}

function CreditCardVisual({ student, showCard, flipped, onFlip }) {
  ...
  return (
    ...
    <div className="relative w-full h-full rounded-[13px] overflow-hidden" style={{
      ...bgStyle, color: textColor, backfaceVisibility: 'hidden',
    ...
    {!useCustomImage && <CardBack student={student} design={design} light={light} />}
    </div>
    </div>
  );
}
...
function StudentView({ student, data, saveData, onLogout, toast }) {
  ...
  const downloadStatement = () => {
    ...
    const byMonth = {};
    myTx.forEach(t => {
      const k = new Date(t.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
      (byMonth[k] = byMonth[k] || []).push(t);
    });

    const cats = {};
    myTx.filter(t => t.amount < 0).forEach(t => {
      const r = String(t.reason || '');
      const k = r.startsWith('Request') ? 'Rewards' : r.startsWith('Transfer') ? 'Transfers' : r.startsWith('Refund') ? 'Refunds' : 'Other';
      cats[k] = (cats[k] || 0) + Math.abs(t.amount);
    });
    const maxCat = Math.max(1, ...Object.values(cats));

    const rows = Object.entries(byMonth).map(([month, list]) => {
      const inSum = list.filter(t => t.amount > 0).reduce((a, t) => a + t.amount, 0);
      const outSum = list.filter(t => t.amount < 0).reduce((a, t) => a + Math.abs(t.amount), 0);
      return `<div class="mh"><span>${esc(month)}</span><span class="mt">+${inSum.toLocaleString('en-US')} · -${outSum.toLocaleString('en-US')}</span></div>` +
        list.map(t => `<div class="tx"><div><div class="r">${esc(t.reason)}</div><div class="d">${new Date(t.date).toLocaleString('en-US')}</div></div><div class="a ${t.amount > 0 ? 'pos' : 'neg'}">${money(t.amount)}</div></div>`).join('');
    }).join('');

    const bars = Object.entries(cats).sort((a, b) => b[1] - a[1]).map(([k, v]) =>
      `<div class="cat"><div class="cr"><span>${esc(k)}</span><span>${v.toLocaleString('en-US')}</span></div><div class="bar"><i style="width:${Math.round(v / maxCat * 100)}%"></i></div></div>`).join('') || '<div class="empty">No spending recorded.</div>';

    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>P4 Reserve Statement — ${esc(student.displayName || student.name)}</title>
<style>
*{box-sizing:border-box}
body{margin:0;background:#0B0710;color:#F3EFF7;font-family:'Manrope',-apple-system,Segoe UI,Helvetica,Arial,sans-serif;padding:32px 18px}
.w{max-width:760px;margin:0 auto}
.hd{text-align:center;padding-bottom:26px;border-bottom:1px solid rgba(255,255,255,.12)}
.bm{font-size:15px;letter-spacing:.26em;color:#D9BE88;font-weight:600}
.sb{font-size:11px;letter-spacing:.22em;color:rgba(255,255,255,.4);margin-top:6px}
.who{margin-top:22px;font-size:26px;font-weight:600}
.meta{font-size:12px;color:rgba(255,255,255,.42);margin-top:6px}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin:26px 0}
.kpi{border:1px solid rgba(255,255,255,.10);border-radius:16px;padding:16px;background:rgba(255,255,255,.025)}
.kl{font-size:10px;letter-spacing:.18em;color:rgba(255,255,255,.4);text-transform:uppercase}
.kv{font-size:24px;font-weight:600;margin-top:6px}
.gold{color:#D9BE88}.pos{color:#6FCF97}.neg{color:#FF8A96}
h2{font-size:12px;letter-spacing:.2em;color:rgba(255,255,255,.45);text-transform:uppercase;margin:34px 0 12px;font-weight:500}
.cat{margin-bottom:13px}
.cr{display:flex;justify-content:space-between;font-size:13px;margin-bottom:5px;color:rgba(255,255,255,.82)}
.bar{height:6px;background:rgba(255,255,255,.07);border-radius:3px;overflow:hidden}
.bar i{display:block;height:100%;background:#D9BE88}
.mh{display:flex;justify-content:space-between;align-items:baseline;margin:22px 0 8px;font-size:14px;font-weight:600}
.mt{font-size:11px;color:rgba(255,255,255,.4);font-weight:400}
.tx{display:flex;justify-content:space-between;gap:14px;padding:11px 0;border-top:1px solid rgba(255,255,255,.07)}
.r{font-size:13px}.d{font-size:11px;color:rgba(255,255,255,.36);margin-top:2px}
.a{font-size:14px;font-weight:600;white-space:nowrap}
.empty{font-size:13px;color:rgba(255,255,255,.36)}
.ft{margin-top:40px;padding-top:20px;border-top:1px solid rgba(255,255,255,.12);text-align:center;font-size:11px;color:rgba(255,255,255,.3)}
@media print{body{background:#fff;color:#111}.kpi,.bar{border-color:#ddd}.hd,.tx,.ft{border-color:#ddd}}
</style></head><body><div class="w">
<div class="hd"><div class="bm">P4 RESERVE</div><div class="sb">ACCOUNT STATEMENT</div>
<div class="who">${esc(student.displayName || student.name)}</div>
<div class="meta">Member ${esc(memberNumber)} · ${esc(student.accessCode)} · ${esc(status.name)} · Issued ${now.toLocaleString('en-US')}</div></div>
<div class="grid">
<div class="kpi"><div class="kl">Current balance</div><div class="kv gold">${student.balance.toLocaleString('en-US')}</div></div>
<div class="kpi"><div class="kl">Total earned</div><div class="kv pos">${totalEarned.toLocaleString('en-US')}</div></div>
<div class="kpi"><div class="kl">Total spent</div><div class="kv neg">${totalSpent.toLocaleString('en-US')}</div></div>
<div class="kpi"><div class="kl">Cards unlocked</div><div class="kv">${unlockedDesigns.length}/${CARD_DESIGNS.length}</div></div>
</div>
<h2>Spending breakdown</h2>${bars}
<h2>Transactions (${myTx.length})</h2>${rows || '<div class="empty">No transactions yet.</div>'}
<div class="ft">Classroom simulation · Not a financial instrument · P4 Reserve</div>
</div></body></html>`;

    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([html], { type: 'text/html;charset=utf-8' }));
    a.download = `p4-statement-${student.accessCode}-${now.toISOString().slice(0, 10)}.html`;
    a.click();
    URL.revokeObjectURL(a.href);
    toast && toast('Statement downloaded. Open it to print or save as PDF.');
  };
  ...
  const handlePhotoUpload = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.size > 10 * 1024 * 1024) { toast && toast('Image too large. Max 10MB.', 'bad'); e.target.value = ''; return; }
    setUploadingPhoto(true);
    try {
      const square = await compressSquare(file);
      if (square.length > 300000) { toast && toast('Image still too large.', 'bad'); setUploadingPhoto(false); e.target.value = ''; return; }
      await updateMyData({ profilePhoto: square });
      toast && toast('Profile photo updated');
    } catch (err) {
      console.error(err); toast && toast('Could not process image.', 'bad');
    }
    setUploadingPhoto(false); e.target.value = '';
  };

  const removePhoto = () => { updateMyData({ profilePhoto: null }); toast && toast('Photo removed'); };
  ...
  const renderHome = () => (
    <div className="space-y-5 pb-28">
      ...
      <section className="relative">
        <CreditCardVisual student={student} showCard={showCard} flipped={flipped} />
        {student.frozen && (
        ...
        <div className="absolute top-3 right-3 z-20 flex gap-2">
          <button onClick={() => setFlipped(!flipped)}
            ...
            style={{background:'rgba(0,0,0,.42)', backdropFilter:'blur(6px)', color:'rgba(255,255,255,.85)'}}
            aria-label="Flip card"><RotateCcw size={14}/></button>
  ...
  const renderCard = () => (
    <div className="pb-28 pt-2">
      <SectionTitle eyebrow="WALLET" title="Your card" action={<div className="flex gap-2"><button onClick={()=>setFlipped(!flipped)} className="w-9 h-9 rounded-xl border flex items-center justify-center" style={{borderColor:theme.border,color:theme.primary}} aria-label="Flip card"><RotateCcw size={15}/></button><button onClick={()=>setShowCard(!showCard)} className="w-9 h-9 rounded-xl border flex items-center justify-center" style={{borderColor:theme.border,color:theme.primary}}>{showCard?<EyeOff size={15}/>:<Eye size={15}/>}</button></div>} />
  ...
  const renderProfile = () => (
    <div className="pb-28 pt-2">
      ...
      <SectionTitle eyebrow="IDENTITY" title="Profile photo"/>
      <div className="flex items-center gap-4 mb-7">
        <Avatar student={student} size={76} ring={theme.primary} />
        <div className="flex-1 min-w-0">
          <div className="text-xs mb-1" style={{color:theme.muted}}>
            {student.profilePhoto ? 'Your photo is visible to the class.' : 'Add a photo. Otherwise your initials are shown.'}
          </div>
          <input ref={photoInputRef} type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden"/>
          <div className="flex gap-2 mt-2">
            <button onClick={()=>photoInputRef.current?.click()} disabled={uploadingPhoto}
              className="rounded-2xl border px-4 py-2.5 text-xs font-semibold flex items-center gap-2"
              style={{borderColor:theme.border,color:theme.accent,opacity:uploadingPhoto?.5:1}}>
              <Upload size={14}/>{uploadingPhoto ? 'PROCESSING' : (student.profilePhoto ? 'CHANGE' : 'UPLOAD')}
            </button>
            {student.profilePhoto && (
              <button onClick={removePhoto} className="rounded-2xl border w-11 flex items-center justify-center"
                style={{borderColor:theme.border,color:theme.danger}}><Trash2 size={14}/></button>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen" style={{fontFamily:FONT_BODY, background:theme.bgGradient, color:theme.accent}}>
      ...
      {receipt && <Receipt data={receipt} theme={theme} toast={toast} onClose={() => setReceipt(null)} />}
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <Avatar student={student} size={44} ring="rgba(217,162,39,.45)" />
                          <div>
